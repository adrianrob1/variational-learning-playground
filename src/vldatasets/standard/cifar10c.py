...
class CIFAR10C(VisionDataset):
    def __init__(
        ...
        if severity not in (None,) + SEVERITY_LEVELS:
            raise RuntimeError(f"severity can only range from 1~5, found {severity}")
        if download:
            _download_cifar10c(root)
        self._dpath = ""
        self._doff = self._check_and_get_offset(root, corruption, severity)
        self._dshape = (50000, 32, 32, 3) if severity is None else (10000, 32, 32, 3)
    ...
    def _get_data(self, index: Union[int, List[int]]):
        ...
        if isinstance(index, int):
            ...
            return torch.stack(
                [self_transform(imgfromarray(d)) for d in data.reshape(-1, 32, 32, 3)],
                dim=0,
            )
...
class MultiCIFAR10C(VisionDataset):
    ...
    def _get_multidata(self, lidx: List[int]):
        ...
        _d = multidata[0][1]
        out = torch.empty((len(lidx), *_d.size()[1:]), dtype=_d.dtype, device=_d.device)
