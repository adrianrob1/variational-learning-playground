...
class IVONFederated(torch.optim.Optimizer):
    ...
    def _set_posterior_simple(self, posterior_mean, posterior_prec):
        for group in self.param_groups:
            ...
            # set hessian
            group["hess"] = (
                (posterior_prec / group["ess"]) - group["weight_decay"] - self.local_wd
            )
    ...
    def _get_posterior(self):
        ...
        for group in self.param_groups:
            mean.append(
                torch.cat([p.flatten() for p in group["params"] if p is not None])
            )
            prec.append(
    ...
    def _sample_params(self) -> Tuple[Tensor, Tensor]:
        ...
        for group in self.param_groups:
            ...
            noise_sample = (
                ...
                / (
                    group["ess"]
                    * (group["hess"] + group["weight_decay"] + self.local_wd)
                ).sqrt()
            )
    ...
    def _new_param_averages(
        ...
        if prior_mean is None:
            return param_avg - lr * torch.clip(
                (momentum / debias + lagr_grad + wd * param_avg + loc_wd * param_avg)
                / (hess + wd + loc_wd),
                min=-clip_radius,
                max=clip_radius,
            )
