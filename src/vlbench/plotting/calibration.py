...
"""Calibration metrics and reliability diagram plotting."""
...
from vlbench.train.utils import autoinitcoroutine
...
def bins2ece(bins: BINSTYPE) -> float:
    """Compute Expected Calibration Error from binned predictions.

    Args:
        bins: Tuple of (bincounts, corrects, cumconf) as returned by data2bins.
...
def bins2acc(bins: BINSTYPE) -> float:
    """Compute accuracy from binned predictions."""
...
def data2bins(rightconfs: Iterable[Tuple[bool, float]], nbin: int = 10) -> BINSTYPE:
    ...
    Args:
        rightconfs: Iterable of (is_correct, confidence) pairs.
        nbin: Number of bins.

    Returns:
        Tuple of (bincounts, corrects, cumconf).
    """
    bincounts = [0] * nbin
    corrects = [0] * nbin
    cumconf = [0.0] * nbin
...
@autoinitcoroutine
def coro_binsmerger():
    """Coroutine that accumulates and merges calibration bins."""
    bins = None
    try:
        bins = yield
        while True:
...
def joinbins(*binses):
    """Merge multiple bin tuples element-wise."""
    lb = len(binses[0][0])
    for bins in binses:
        if len(bins[0]) != lb:
